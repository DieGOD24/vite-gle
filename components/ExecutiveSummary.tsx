...
const ExecutiveSummary: React.FC<ExecutiveSummaryProps> = ({ visits, prospects, closures, previousData, user, users, selectedCommercialName, selectedRegion, dateRange }) => {
    
    const mainChartData = useMemo(() => {
        // If a commercial is selected or the user is a commercial, show their daily visits
        if (selectedCommercialName || user.cargo === Role.COMERCIAL) {
            // Fix: Explicitly type the reduce accumulator to allow indexing by a date string.
             // FIX: Explicitly type the reduce accumulator to fix indexing errors.
             // FIX: Cast the initial value of reduce to Record<string, number> to fix indexing errors.
             const counts = visits.reduce((acc: Record<string, number>, visit) => {
                const date = new Date(visit.fecha_hora).toLocaleDateString('es-CO');
                acc[date] = (acc[date] || 0) + 1;
                return acc;
            }, {} as Record<string, number>);
            const parseDMY = (dateString: string) => { const [day, month, year] = dateString.split('/').map(Number); return new Date(year, month - 1, day); };
            return Object.entries(counts).map(([name, count]) => ({ name, visitas: count })).sort((a,b) => parseDMY(a.name).getTime() - parseDMY(b.name).getTime());
        }
        
        // For managers, show total visits by region
        const userRegionMap = new Map(users.map(u => [u.cedula, u.regional]));
        // Fix: Explicitly type the reduce accumulator. This ensures `visitas` is inferred as a number, fixing the arithmetic error in the subsequent `.sort()` call.
        // FIX: Explicitly type the reduce accumulator to correctly infer types for the subsequent sort operation.
        // FIX: Cast the initial value of reduce to Record<string, number> to fix type inference for the subsequent sort operation.
        const countsByRegion = visits.reduce((acc: Record<string, number>, visit) => {
    ...
    // --- New Financial KPIs ---
    const totalSalesValue = useMemo(() => closures.reduce((sum: number, c: Closure) => sum + c.valor, 0), [closures]);
    const prevTotalSalesValue = useMemo(() => previousData.closures.reduce((sum: number, c: Closure) => sum + c.valor, 0), [previousData.closures]);
    ...
    const visitsByType = useMemo(() => {
        // Fix: Explicitly type the reduce accumulator to allow indexing by the `VisitType` enum.
        // FIX: Explicitly type the reduce accumulator to fix indexing errors.
        // FIX: Cast the initial value of reduce to Record<VisitType, number> to fix indexing errors.
        const counts = visits.reduce((acc: Record<VisitType, number>, visit) => {
            acc[visit.motivo] = (acc[visit.motivo] || 0) + 1;
            return acc;
    ...
    const regionalPerformanceData = useMemo(() => {
        ...
        // Fix: Explicitly type the reduce accumulator to allow creating and accessing its properties.
        // FIX: Explicitly type the reduce accumulator to fix property assignment errors.
        // FIX: Cast the initial value of reduce to the correct type to fix property assignment errors.
        const dataByRegion = REGIONS.reduce((acc: Record<string, { name: string; Prospectos: number; Cierres: number }>, region) => {
            acc[region] = { name: region, Prospectos: 0, Cierres: 0 };
            return acc;
        }, {} as Record<string, { name: string; Prospectos: number; Cierres: number }>);

        prospects.forEach(p => {
            const region = userRegionMap.get(p.cedula_comercial);
            if (region && dataByRegion[region]) {
                dataByRegion[region].Prospectos++;
            }
        });
    ...
    const activityByDay = useMemo(() => {
        ...
        const processItems = (items: (Visit | Prospect | Closure)[], type: 'Visitas' | 'Prospectos' | 'Cierres') => {
            items.forEach(item => {
                const itemDate = 'fecha_hora' in item ? item.fecha_hora : 'fecha_registro' in item ? item.fecha_registro : item.fecha_cierre;
                const dateKey = new Date(itemDate).toISOString().split('T')[0];
                if (!dataByDate.has(dateKey)) {
                    dataByDate.set(dateKey, { date: dateKey, displayDate: new Date(itemDate).toLocaleDateString('es-CO', { day: '2-digit', month: 'short' }), Visitas: 0, Prospectos: 0, Cierres: 0 });
                }
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                <div className="bg-white p-6 rounded-lg shadow-md lg:col-span-3">
                    ...
                    <ResponsiveContainer width="100%" height={300}>
                        <BarChart data={mainChartData} layout={user.cargo === Role.COMERCIAL || selectedCommercialName ? "horizontal" : "vertical"} margin={{ top: 5, right: 30, left: (user.cargo === Role.COMERCIAL || selectedCommercialName ? 20 : 60), bottom: 5 }}>
