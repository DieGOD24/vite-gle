...
const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  // ... (hooks useState, useEffect, etc. sin cambios) ...
  ...
  const { filteredVisits, filteredProspects, filteredClosures, previousPeriodData } = useMemo(() => {
    // CORRECCIÓN: Añadimos .filter(Boolean) para limpiar nulos
  ...
  // --- CORRECCIÓN DEL BUG ---
  // Este 'useMemo' estaba causando el "choque" de la aplicación.
  const clientsForView = useMemo(() => {
    const clientMap = new Map<string, { name: string; nit: string }>();
    
    // CORRECCIÓN: Filtramos cualquier posible 'null' o 'undefined' de las props
    ...
    interactions.forEach(item => {
        ...
        // Lógica corregida para encontrar el nombre basado en types.ts
        if ('nombre_cliente' in item) {
            name = item.nombre_cliente; // Es una Visita
        } else if ('nombre_empresa' in item) {
            name = item.nombre_empresa; // Es un Prospecto
        } else if ('cliente' in item) {
            name = item.cliente; // Es un Cierre
        }

        // Solo añadir al mapa si encontramos un nombre y un NIT
        if (name && item.nit && !clientMap.has(item.nit)) {
            clientMap.set(item.nit, { name, nit: item.nit });
        }
    });
    
    // Esto ahora es seguro y no fallará
